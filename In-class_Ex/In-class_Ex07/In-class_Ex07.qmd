...
tmap_mode("plot") # Interactive (zoomable) mode
...
This data represents the GWR model output for a single observation point within the Tampines Planning Area.

#### 1. Model Goodness-of-Fit (Local R²)

-   **Local_R²: 0.972431**

-   **Interpretation:** The Local R-squared value at this point is **0.972 (or 97.2%)**, which signifies an **exceptionally high**level of explanatory power. This means the GWR model successfully explains 97.2% of the variance in the housing price at this specific location.

#### 2. Price Prediction and Residual

-   **Actual Price (y):** \$1,095,000

-   **Predicted Price (yhat):** \$1,106,570

-   **Residual:** -\$11,570

-   **Interpretation:** The model **over-predicted** the property's actual price by \$11,570. The negative residual indicates that the predicted value (yhat) was higher than the actual observed value (y).

#### 3. Conclusion (Overall Interpretation)

This observation point serves as an **excellent case study for the model's effectiveness.**

The extremely high Local R² (97.2%) indicates that the model has captured the local price-determining factors almost perfectly.

This conclusion is strongly supported by the **very low residual**. For a property valued at over \$1 million, the model's prediction was off by only \$11,570, representing a minimal error rate of just **1.06%**. This demonstrates that the model not only fit the data well (high R²) but was also **highly accurate** in its prediction for this location.

...
1.  Convert **GIS polygon data** into an **R simple feature data.frame** using the `sf` package.

2.  Transform a **simple feature data.frame** into an **R SpatialPolygonDataFrame object** with the `sf` package.

3.  Perform **cluster analysis** using the `hclust()` function in Base R.

4.  Conduct **spatially constrained cluster analysis** using the `skater()` function in Base R.

5.  **Visualize analysis outputs** with the `ggplot2` and `tmap` packages.

## **12.2 Getting Started**

### **12.2.1 The analytical question**

In **geobusiness** and **spatial policy**, it is common to divide markets or planning areas into **homogeneous regions** using multivariate data.

For this exercise, we focus on **Shan State, Myanmar**, and aim to delineate it into homogeneous regions based on multiple **ICT (Information and Communication Technology) measures**:
...
```{r}
pacman::p_load(spdep, tmap, sf, ClustGeo, dplyr,
...
-   **Problem:** ICT values are raw household counts → larger townships naturally show higher values (bias).

-   **Solution:** Compute **penetration rates** for each ICT variable (Radio, TV, Landline, Mobile, Computer, Internet) by dividing the number of households owning ICT by the total households.

-   **Purpose:** Removes bias from township size, making results comparable across regions.
...
-   **Purpose:** To prepare for a **choropleth map**, we need to merge geospatial data (`shan_sf`) with aspatial data (`ict_derived`).

-   **Method:** Use `left_join()` from the **dplyr** package.

-   **Base data:** `shan_sf` (simple feature data.frame).

-   **Join table:** `ict_derived` (aspatial data.frame).

-   **Key (unique identifier):** `TS_PCODE`.
...
#| fig-width: 12
#| fig-height: 6
qtm(shan_sf, "RADIO_PR", fill.palette = "YlOrBr")
```
...
**Answer:**\
The TT_HOUSEHOLDS map shows the total number of households, so areas with larger populations naturally appear darker. In contrast, the RADIO_PR map shows the penetration rate of radio ownership, which controls for township size and highlights where radio adoption is relatively high or low.
:::
...
-   Cluster analysis uses multiple variables with different value ranges.

-   Variables with larger ranges can dominate the results → **bias**.

-   Therefore, standardisation is needed to put all variables on a **comparable scale** before clustering.
...
> **Answer:**
>
> The histograms show that standardisation changes the scale of the variable but does not alter its underlying distribution.
>
> -   The raw values of `RADIO_PR` range widely (0–500).
>
> -   After **Min-Max standardisation**, the values are rescaled between 0 and 1 while keeping the same distributional shape.
>
> -   After **Z-score standardisation**, the variable has mean 0 and standard deviation 1, but again the overall distribution remains the same.
>
> **Conclusion:** Standardisation only rescales the data to comparable units, ensuring that no variable dominates the analysis due to its scale, while the original distribution pattern is preserved.
> :::
...
::: callout-note
The **NbClust** package (Charrad et al., 2014) offers **30 different indices** to determine the optimal number of clusters. It evaluates all combinations of cluster numbers, distance measures, and clustering methods, and then recommends the **best clustering scheme** based on the results.
:::
...
-   Moving **up the tree**, similar observations are merged into branches, which are then fused at higher levels.

-   The **height on the vertical axis** indicates dissimilarity:

    -   **Lower fusion height** → observations are more similar.

    -   **Higher fusion height** → observations are less similar.

-   Important: Only the **vertical height** matters for similarity, not the horizontal position.

-   To highlight clusters, [*rect.hclust()*](https://stat.ethz.ch/R-manual/R-devel/library/stats/html/rect.hclust.html) can be used to draw rectangles around groups, with the `border` argument specifying rectangle colors.
...
```{r}
library(tmap)  
qtm(shan_sf_cluster, fill = "CLUSTER", fill.palette = "Set2")
...
You can visualize the neighbors list on `shan_sp` by following these steps:

1.  First, plot the **boundaries** to create the base map.

2.  Then, plot the **neighbors list** on top of it. For this, the **centroids** of the original polygons (Shan state township boundaries) are extracted and used as the nodes of the graph.

3.  The network edges are drawn in **blue**, and the `add=TRUE` option ensures the network is overlaid on the existing map.

**In short:** draw the map boundaries first, then overlay the township neighbor relationships as a centroid-based network.
...
```{r}
shan_sf_ngeo_cluster <- cbind(shan_sf, as.matrix(groups)) %>% rename(CLUSTER = as.matrix.groups.)
```

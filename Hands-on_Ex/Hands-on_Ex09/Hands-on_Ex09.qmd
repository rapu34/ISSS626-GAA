...
```{r}
# Convert from long to wide format (prevent printing intermediate objects)
...
# Set origin_id as row names to create a matrix-like numeric table
distmat_mat <- distmat_wide %>%
  column_to_rownames("origin_id") %>%
  as.matrix()

# Display as a single formatted table (same layout as shown in lecture slides)
kable(
  round(head(distmat_mat, 10)[ , 1:min(10, ncol(distmat_mat))], 2),
  align = "r",
  caption = "Preview of total-cost O–D matrix (10×10)"
)
...
```{r}
#Memory issue show n=10 
acc_Hansen %>% slice_head(n = 10)
```
...
U might need to check the rendering issue.

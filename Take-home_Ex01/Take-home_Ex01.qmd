...
```{r, fig.width=15, fig.height=5, dev="png"}
...
```{r, fig.width=15, fig.height=10, dev="png"}
...
**Figure 3.6: Top 3 Regional Business Distribution Analysis**
...
```{r, fig.width=15, fig.height=12, dev="png"}
...
**Figure 3.7: Regional Clustering Pattern Analysis**
...
```{r}
# Load required packages
pacman::p_load(spdep, classInt, sp)
...
# Create distance weights (using 3km threshold for Singapore)
distance_weights <- dnearneigh(coords, 0, 3, longlat = FALSE)
...
Contiguity weights visualization displays the spatial neighborhood relationships defined by different contiguity criteria, enabling visual inspection of spatial weights matrix structures. The `st_centroid()` function calculates subzone centroids for connection visualization, while `st_coordinates()` extracts coordinate pairs for plotting. The `par(mfrow = c(1, 2))` function creates side-by-side plots for comparing different contiguity types. The `plot()` function with `st_geometry()` renders subzone boundaries, while `plot.nb()` overlays neighborhood connections as lines between centroids. The `col = "red"` and `lwd = 0.5` parameters create visible but non-overwhelming connection lines, enabling clear visualization of spatial neighborhood structures.
...
# Create Queen contiguity visualization
...
# Create distance-based weights visualization
plot(st_geometry(singapore_subzones_density), 
     border = "lightgrey", 
     main = "Distance-based Weights (3km threshold)",
     cex.main = 1.2)
...
### **5.5 Subzone-level Detailed Analysis**
...
```{r, fig.width=15, fig.height=10, dev="png"}
# Create detailed subzone analysis maps
library(tmap)
...
## **6. Advanced Visualization and Interactive Analysis**

Advanced visualization and interactive analysis employs cutting-edge geospatial visualization techniques to enhance understanding of spatial patterns and enable dynamic exploration of business distribution data. This section demonstrates proficiency in advanced mapping technologies, including interactive web-based maps, animated visualizations, and dynamic statistical graphics that provide deeper insights into spatial clustering patterns and temporal trends.

### **6.2 Animated Spatio-temporal Maps**

Animated spatio-temporal maps create dynamic visualizations that reveal temporal changes in spatial patterns, enabling visualization of how business clustering patterns evolve over time. The `gganimate` package provides functions for creating animated visualizations, while `transformr` enables smooth interpolation between animation states. The `st_drop_geometry()` function removes spatial attributes for temporal data processing, while `mutate()` and `month()` functions extract temporal components for animation frames. The `ggplot()` function creates base visualizations with `geom_bar()` for temporal trends and `geom_point()` for spatial distributions. The `transition_states()` function creates animation frames based on temporal states, while `animate()` generates final animated outputs with specified duration and frame rates.

```{r fig.height=8, fig.width=12, dev="png"}
#| eval: false
#| include: false
# Load required packages
library(gganimate)
...
animation_data <- acra_innovation_sf %>%
  st_drop_geometry() %>%
  mutate(
    reg_month = month(registration_incorporation_date),
    reg_month_name = month(registration_incorporation_date, label = TRUE, abbr = TRUE)
  ...
  group_by(reg_month, reg_month_name, sector) %>%
  summarise(count = n(), .groups = "drop")

# Create animated bar chart
animated_bar <- ggplot(animation_data, aes(x = sector, y = count, fill = sector)) +
  geom_bar(stat = "identity") +
  scale_fill_manual(values = c("Technology" = "blue", "Finance" = "red", "R&D" = "green")) +
  labs(title = "Monthly Business Registration Trends: {closest_state}",
       x = "Sector", y = "Number of New Businesses") +
  theme_minimal() +
  transition_states(reg_month_name, transition_length = 2, state_length = 1) +
  ease_aes("cubic-in-out")

# Animate the plot
gganimate::animate(animated_bar, nframes = 50, fps = 5, width = 400, height = 300)

# Create animated scatter plot
coords_anim <- acra_innovation_sf %>%
  mutate(
    reg_month = month(registration_incorporation_date),
    reg_month_name = month(registration_incorporation_date, label = TRUE, abbr = TRUE)
  ) %>%
  filter(!is.na(reg_month)) %>%
  st_drop_geometry() %>%
  cbind(st_coordinates(acra_innovation_sf))

animated_scatter <- ggplot(coords_anim, aes(x = X, y = Y, color = sector)) +
  geom_point(alpha = 0.7, size = 1) +
  scale_color_manual(values = c("Technology" = "blue", "Finance" = "red", "R&D" = "green")) +
  labs(title = "Spatial Distribution of New Businesses: {closest_state}",
...
gganimate::animate(animated_scatter, nframes = 50, fps = 5, width = 400, height = 300)
```
